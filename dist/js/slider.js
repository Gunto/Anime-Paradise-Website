(function($) {
  ...
  Slider = (function() {
    function Slider(el, settings) {
      ...
      (_.defaults = {
        ...
        buttons: {
          prev:
            "<div class='prev slider-buttons'><i class='fas fa-chevron-left'></i></div>",
          next:
            "<div class='next slider-buttons'><i class='fas fa-chevron-right'></i></div>"
        }
      }),
        (_.markup = {
          $slider: $(el),
  ...
  Slider.prototype.setup = function() {
    ...
    _.markup.$slider.append(
      _.markup.$slider.children("li")[_.options.initialslide - 1].outerHTML
    );
    ...
    if (_.options.navigation === true) {
      _.markup.$slidercontainer.append(
        _.options.buttons.prev,
        _.options.buttons.next
      );
    ...
    if (_.options.pagination === true) {
      ...
      _.markup.$slidercontainer.append(
        "<ul class='pagination-container'>" + pagination + "</ul>"
      );

      _.markup.$dotscontainer = _.markup.$slidercontainer.find(
        ".pagination-container"
      );

      // set first slide active
      _.markup.$dotscontainer
        .children("li")
        .eq(0)
        .addClass("active");
  ...
  Slider.prototype.responsive = function() {
    ...
    if (
      minimumHeight < parseInt(_.options.height, 10) ||
      _.options.height === "auto"
    ) {
      _.markup.$slidercontainer.height(minimumHeight);
    }
  ...
  Slider.prototype.slide = function(immediate, direction, frame) {
    ...
    if (immediate === true) {
      ...
      _.interval = setInterval(
        $.proxy(_.slideTransition, _, direction),
        _.options.delay
      );
    }
  };
